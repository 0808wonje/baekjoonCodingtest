...
public class No7569 {
  public static void main(String[] args) throws IOException {
    ...
    Tomato[][][] arr = new Tomato[height][row][col];
      ...
      for (int i = 0; i < row; i++) {
        ...
        for (int j = 0; j < col; j++) {
          if (Integer.parseInt(tomatoes[j]) == 1) {
            arr[k][i][j] = new Tomato(true);
          } else if (Integer.parseInt(tomatoes[j]) == 0) {
            arr[k][i][j] = new Tomato(false);
          } else {
            arr[k][i][j] = null;
          }
        }
    ...
    for (int k = 0; k < height; k++) {
      for (int i = 0; i < row; i++) {
        for (int j = 0; j < col; j++) {
          ...
          if (j - 1 >= 0) {
            arr[k][i][j].setLeft(arr[k][i][j - 1]);
          }
          if (j + 1 < col) {
            arr[k][i][j].setRight(arr[k][i][j + 1]);
          }
          if (i - 1 >= 0) {
            arr[k][i][j].setUp(arr[k][i - 1][j]);
          }
          if (i + 1 < row) {
            arr[k][i][j].setDown(arr[k][i + 1][j]);
          }
          if (k - 1 >= 0) {
            arr[k][i][j].setUpperLevel(arr[k - 1][i][j]);
          }
          if (k + 1 < height) {
            arr[k][i][j].setDownLevel(arr[k + 1][i][j]);
          }
        }
      }
    }
    ...
    for (int k = 0; k < height; k++) {
      for (int i = 0; i < row; i++) {
        for (int j = 0; j < col; j++) {
          if (arr[k][i][j] != null && arr[k][i][j].isRipe()) {
            queue.add(arr[k][i][j]);
          }
        }
      }
    ...
    for (int k = 0; k < height; k++) {
      ...
      for (int i = 0; i < row; i++) {
        for (int j = 0; j < col; j++) {
          ...
          if (arr[k][i][j].getDay() > result) {
            result = arr[k][i][j].getDay();
          }
        }
        if (flag) {
          break;
    ...
    if (answer.contains(-1)) {
      ...
      int day = answer.stream().mapToInt(Integer::intValue).max().getAsInt();
      bw.write(day + "\n");
      bw.flush();
    }
  }
  ...
  static class Tomato {
    ...
    private Tomato upperLevel;
    private Tomato downLevel;
    private int day;

    public Tomato getDownLevel() {
      return downLevel;
    }

    public void setDownLevel(Tomato downLevel) {
    ...
    public Tomato getUpperLevel() {
      return upperLevel;
    }

    public void setUpperLevel(Tomato upperLevel) {
  ...
  static void search(Queue<Tomato> root) {
    ...
    while (!queue.isEmpty()) {
      ...
      if (tomato.getUpperLevel() != null && !tomato.getUpperLevel().isRipe()) {
        tomato.getUpperLevel().setRipe(true);
        tomato.getUpperLevel().setDay(tomato.getDay() + 1);
        queue.add(tomato.getUpperLevel());
      }
      if (tomato.getDownLevel() != null && !tomato.getDownLevel().isRipe()) {
        tomato.getDownLevel().setRipe(true);
        tomato.getDownLevel().setDay(tomato.getDay() + 1);
        queue.add(tomato.getDownLevel());
      }
    }
  }
}
